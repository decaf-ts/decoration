...
describe('integration: decoration builder end-to-end', () => {
  it('applies default base decorators and flavoured extras resolved at application time', () => {
    ...
    const extraMethodDeco: MethodDecorator = (_t: any, key: string | symbol) => {
      applied.push(`extra-method:${String(key)}`);
    };

    // 1) Register default base decorators for key 'thing'
    const defaultBuilder = new Decoration().for('thing').define(baseClassDeco, basePropDeco);
    ...
    @deco
    class X {
      @((deco as unknown) as PropertyDecorator)
      p!: number;

      @((deco as unknown) as MethodDecorator)
      m(): void {}
    }

    // Verify that expected decorators were applied (ordering differs by invocation semantics)
    expect(applied).toEqual(expect.arrayContaining(['base-class', 'base-prop:p', 'extra-method:m']));
  });

  it('apply() throws when no key was provided', () => {
    const builder = new Decoration();
    expect(() => builder.apply()).toThrow('No key provided for the decoration builder');
  });
  ...
  it('register() throws when decorators or flavour are missing', () => {
    ...
    const callRegister = () => (Decoration as any).register('k', '', undefined, undefined);
    expect(callRegister).toThrow('No decorators provided for the decoration builder');

    // now provide empty set but missing flavour should still throw
    const callRegister2 = () => (Decoration as any).register('k', '' as any, new Set(), undefined);
    expect(callRegister2).toThrow('No flavour provided for the decoration builder');
  });
});
...
describe('integration: library registration', () => {
  it('registerLibrary should throw on duplicate registration', () => {
    ...
    expect(() => Metadata.registerLibrary(key, '1.0.1')).toThrow(/already .* registered/i);
  });
});
